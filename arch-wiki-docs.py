...
if __name__ == "__main__":
    ...
    aparser.add_argument(
        "--clean",
        action="store_true",
        help="Clean the output directory after downloading, useful for removing pages deleted/moved on the wiki. Warning: any unknown files found in the output directory will be deleted!",
    )
    aparser.add_argument(
        "--safe-filenames",
        action="store_true",
    ...
    aparser.add_argument(
        "--list-langs", action="store_true", help="List supported languages"
    )
    ...
    aw = ArchWiki.ArchWiki(
        user_agent=user_agent, safe_filenames=args.safe_filenames, langs=args.langs
    )
    optimizer = ArchWiki.Optimizer(aw, args.output_directory)

    downloader = ArchWiki.Downloader(
        aw, args.output_directory, epoch, optimizer=optimizer
    )
